...
function handleContactSubmit(e) {
    ...
    const emailInput = document.querySelector('.contact-form input[type="email"]')?.value?.trim() || '';
    const subject = document.querySelectorAll('.contact-form input[type="text"]')[1]?.value?.trim() || 'Portfolio Contact';
    const message = document.querySelector('.contact-form textarea')?.value?.trim() || '';
    const body = encodeURIComponent(`Name: ${name}\nEmail: ${emailInput}\n\n${message}`);
    const mailto = `mailto:haenainglay20@gmail.com?subject=${encodeURIComponent(subject)}&body=${body}`;
